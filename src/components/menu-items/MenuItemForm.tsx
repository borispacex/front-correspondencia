...
export default function MenuItemForm({ item, allItems = [], onSubmit, onCancel }: MenuItemFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <Label>
          Etiqueta <span className="text-error-500">*</span>
        </Label>
        <InputField
          value={values.label}
      ...
      <div>
        <Label>
          Icono<span className="text-error-500">*</span>
        </Label>
        <div
          className={`mt-2 flex flex-wrap gap-y-2 rounded-lg border px-2 py-2 ${errors.icon ? 'border-error-500 dark:border-error-700' : 'border-gray-200 dark:border-gray-700'}`}
        >
          {ICON_OPTIONS.map(({ value, label: iconLabel, Icon }) => (
            <button
              ...
              title={iconLabel}
              className={`mx-1 flex flex-col items-center justify-center gap-1 rounded-lg border p-2 text-xs transition-colors ${
                values.icon === value
                  ? 'border-brand-500 bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400'
                  : 'hover:border-brand-300 border-gray-200 bg-white text-gray-500 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400'
              }`}
            >
              <Icon className="size-5" />
              <span className="w-full truncate text-center">{iconLabel}</span>
            </button>
          ))}
          <button
            type="button"
        ...
        {errors.icon && <p className="text-error-500 mt-1 text-sm">Icono es requerido</p>}
      </div>

      <div>
        <Label>
          Orden<span className="text-error-500">*</span>
        </Label>
        <InputField
          type="number"
          value={values.order}
      ...
      <div>
        ...
        <Select
          ...
          options={parentOptions.map((opt) => ({
            value: String(opt.id),
            label: opt.parent_id ? `↳ ${opt.label}` : opt.label,
          }))}
        />
      </div>
