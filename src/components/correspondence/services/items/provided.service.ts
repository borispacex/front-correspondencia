...
export async function getProvidedById(id: number): Promise<Provided> {
  const { data } = await http.get<ApiResponse<Provided>>(API_ENDPOINTS.CORRESPONDENCE.PROVIDED.BY_ID(id));
  return data.data;
}
