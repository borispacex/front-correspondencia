...
import {
    CreateRouterRequest,
} from "../../types/routers/router.type.ts";
...
export default function RouterForm({
    ...
    const [stateDocuments, setStateDocuments] =
        useState<Option[]>([]);

    const [provides, setProvides] =
        useState<Option[]>([]);
     ...
     * Checkbox proveidos
     */
    function handleProvidedChange(
        value: string
    ) {

        const exists =
            values.providedIds.includes(value);

        if (exists) {

            setValue(
                "providedIds",
                values.providedIds.filter(
                    (item: string) =>
                        item !== value
                )
            );
    ...
    function validate(): boolean {
        ...
        if (!values.selectedDepartmentDestino) {
            newErrors.selectedDepartmentDestino =
                "Debe seleccionar un departamento destino";
        }

        if (!values.selectedUserDestino) {
            newErrors.selectedUserDestino =
                "Debe seleccionar un usuario destino";
        }

        if (!values.selectedStateDocument) {
            newErrors.selectedStateDocument =
                "Debe seleccionar un estado";
        }

        if (
            !values.selectedProcedureType
        ) {
            newErrors.selectedProcedureType =
                "Debe seleccionar un trámite";
        }

        if (
            values.providedIds.length === 0
        ) {
            newErrors.providedIds =
                "Debe seleccionar al menos un proveído";
        }
    ...
    async function handleSubmit(
        ...
        try {
            ...
            const payload:
                ...
                department_id_destino:
                    Number(
                        values.selectedDepartmentDestino
                    ),

                user_id_destino:
                    values.selectedUserDestino
                        ? Number(
                            values.selectedUserDestino
                        )
                        : undefined,
                ...
                state_document_id:
                    Number(
                        values.selectedStateDocument
                    ),

                provided_ids:
                values.providedIds,

                rout_aclaracion_proveido:
                    values.routAclaracionProveido.trim(),

                rout_observacion:
                    values.routObservacion.trim(),
            };
                ...
                <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
                    ...
                    <div>
                        ...
                        <Select
                            ...
                            error={
                                !!errors.selectedDepartmentDestino
                            }
                            hint={
                                errors.selectedDepartmentDestino
                            }
