...
import {Area, CreateRoadmapRequest, Roadmap, UpdateRoadmapRequest} from "../../../../types/roadmaps/roadmap.type.ts";
...
interface RoadmapFormProps {
    ...
    onSubmit: (
        data: CreateRoadmapRequest | UpdateRoadmapRequest
    ) => Promise<void>;
...
export default function RoadmapForm({
    ...
    useEffect(() => {
        ...
        setDocumentOrigin(
            roadmap?.document_origin ?? "INTERNO"
        );

        setDocumentDate(
            roadmap?.document_date ?? ""
        );
        ...
        if (roadmap?.procedure_type) {

            const foundProcedureType = procedureTypes.find(
                (item) => item.id === roadmap.procedure_type
            ) ?? null;
        ...
        if (roadmap?.priority) {
            ...
            const foundPriority = priorities.find(
                (item) => item.id === roadmap.priority
            ) ?? null;

            setSelectedPriority(foundPriority);
        }

        if (roadmap?.origin_area_id && allAreas.length > 0) {

            const foundArea = allAreas.find(
                (area) => area.id === roadmap.origin_area_id
            ) ?? null;

            setSelectedArea(foundArea);
        }

        if (roadmap?.document_type_id && allDocumentTypes.length > 0) {

            const foundDocumentType = allDocumentTypes.find(
                (item) => item.id === roadmap.document_type_id
            ) ?? null;

            setSelectedDocumentType(foundDocumentType);
        }

    }, [roadmap, allAreas, allDocumentTypes]);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            if (roadmap) {
                ...
                await onSubmit(
                    payload as CreateRoadmapRequest
                );
            }
