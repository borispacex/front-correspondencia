  ...
  return (
    <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-10 dark:border-white/[0.08] dark:bg-white/[0.03]">
      <div className="flex flex-col items-center justify-center text-center">
        ...
        <h3 className="text-sm font-medium text-gray-700 dark:text-gray-200">No existe documentos</h3>

        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Actualiza documentos, por favor.</p>
      </div>
    </div>
  );
};
...
const DocumentDashboardCard = ({ documents }: { documents: Document[] }) => {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-white/[0.05] dark:bg-white/[0.03]">
      <h2 className="mb-5 text-base font-semibold text-gray-800 dark:text-white/90">Panel documentos</h2>
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={i} className="flex items-start justify-between gap-4">
            <span className="w-24 shrink-0 text-xs text-gray-400 dark:text-gray-500">{label}</span>
