...
function Avatar({ name }: { name?: string }) {
  const initials = (name ?? '?')
    ...
    .join('')
    .toUpperCase();
  const colors = ['bg-violet-500', 'bg-sky-500', 'bg-emerald-500', 'bg-rose-500', 'bg-amber-500', 'bg-teal-500'];
...
function DeletedBanner({ onRestore, canRestore }: { onRestore: () => void; canRestore: boolean }) {
  return (
    <div className="flex items-center gap-3 rounded-lg bg-red-50 px-4 py-2 text-sm text-red-600 dark:bg-red-900/20 dark:text-red-400">
      ...
      {canRestore && (
        <Button variant="primary" size="xs" onClick={onRestore} className="ml-auto inline-flex items-center gap-1.5">
        ...
        className={`rounded-xl border p-4 transition-all duration-200 hover:shadow-sm ${
          ...
          isDeleted
            ...
            : `bg-white dark:bg-white/[0.02] ${
                isSelected
                  ? `border-brand-300 ring-brand-500/20 dark:border-brand-500/40 dark:ring-brand-500/30 shadow-md ring-2 dark:shadow-black/20`
                  : `border-gray-100 hover:border-gray-200 dark:border-white/[0.05] dark:hover:border-white/[0.08]`
              } `
        } `}
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <span className="shrink-0 text-right text-xs whitespace-nowrap text-gray-400 dark:text-gray-500">
            {document.doc_fecha_origen ? formatDateBo(document.doc_fecha_origen) : '—'}
          </span>
        </div>
        ...
        {isDeleted && (
          <div className="mt-3">
            <DeletedBanner onRestore={() => onToggleActive(document, true)} canRestore={can('documents.edit')} />
          </div>
        )}
