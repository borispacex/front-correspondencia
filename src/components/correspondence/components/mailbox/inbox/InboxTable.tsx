...
interface Props {
  routers: Router[];
  isLoading?: boolean;
  onViewHeader?: (router: Router) => void;
  onViewRoutes?: (router: Router) => void;
  onDerive?: (router: Router) => void;
  onViewSheet?: (router: Router) => void;
  onView?: (router: Router) => void;
  onArchive?: (id: number) => void;
  onReceive?: (id: number) => void;
  onComment?: (router: Router) => void;
}
                      ...
                      {can('files.view') && onView && (
                        <Tooltip content="Ver tramite">
                          <Button
                            ...
                            startIcon={<EyeIcon className="size-3.5 fill-sky-500 dark:fill-sky-400" />}
                          ></Button>
                        </Tooltip>
                      )}
                      ...
                      {can('files.receive') && onReceive && (
                        <Tooltip content="Recibir">
                          <Button
                            ...
                            onClick={() => onReceive(router.id)}
                          ></Button>
                        </Tooltip>
                      )}
                      {can('files.comment') && onComment && (
                        <Tooltip content="Comentar">
                          <Button
                            variant="info-outline"
                            size="xs"
                            startIcon={<MessageCircleIcon className="size-3.5" />}
                            onClick={() => onComment(router)}
                          ></Button>
                        </Tooltip>
                      )}
                    </div>
