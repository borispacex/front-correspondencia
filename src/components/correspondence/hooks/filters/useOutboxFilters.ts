...
export interface OutboxSortConfig {
  field: string;
  dir: 'asc' | 'desc';
}

const INITIAL_FILTERS: OutboxFilters = {
  nro: '',
  nroCite: '',
  subject: '',
  priority: '',
...
const INITIAL_SORT: OutboxSortConfig = {
  field: 'id',
  dir: 'desc',
};

export function useOutboxFilters(routers: Router[]) {
  const [filters, setFilters] = useState<OutboxFilters>(INITIAL_FILTERS);

  const [sort, setSort] = useState<OutboxSortConfig>(INITIAL_SORT);
