...
const INITIAL_FILTERS: ArchivedFilters = {
  nro: '',
  nroCite: '',
  subject: '',
  priority: '',
...
const INITIAL_SORT: ArchivedSortConfig = {
  field: 'id',
  dir: 'desc',
};

export function useArchivedFilters(routers: Router[]) {
  const [filters, setFilters] = useState<ArchivedFilters>(INITIAL_FILTERS);

  const [sort, setSort] = useState<ArchivedSortConfig>(INITIAL_SORT);
